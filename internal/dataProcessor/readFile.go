...
func ReadJSONFile(dataFile string) (internal.DecisionTree, error) {
	// Open dataFile for reading
	file, err := os.Open(dataFile)
	if err != nil {
		return internal.DecisionTree{}, fmt.Errorf("Error opening JSON file: %v", err)
	}
	defer file.Close()
	...
	if err != nil {
		return internal.DecisionTree{}, fmt.Errorf("Error reading JSON file: %v", err)
	}

	// Unmarshal JSON data into DecisionTree struct
	var trainedModel internal.DecisionTree
	err = json.Unmarshal(data, &trainedModel)
	if err != nil {
		return internal.DecisionTree{}, fmt.Errorf("Error unmarshalling JSON: %v", err)
	}
